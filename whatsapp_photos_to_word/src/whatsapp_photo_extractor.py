...
def interactive_mode():
    ...
    try:
        # Get zip file (default to data/input directory)
        zip_file = input("Enter WhatsApp zip file name (or press Enter for 'WhatsApp Chat.zip'): ").strip()
        if not zip_file:
            zip_file = "WhatsApp Chat.zip"
        
        # If it's just a filename, try data/input first, then current directory
        if not os.path.dirname(zip_file):
            data_input_path = os.path.join("data", "input", zip_file)
            if os.path.exists(data_input_path):
                zip_file = data_input_path
            # Otherwise keep it as current directory
        ...
        # Get output options (default to data/output directory)
        output_doc = input("\nOutput Word document name (or press Enter for 'whatsapp_photos.docx'): ").strip()
        if not output_doc:
            output_doc = "whatsapp_photos.docx"
        
        # If it's just a filename, try to put it in data/output if exists, otherwise current directory
        if not os.path.dirname(output_doc):
            data_output_dir = "data/output"
            if os.path.exists(data_output_dir):
                output_doc = os.path.join(data_output_dir, output_doc)
        
        extract_dir = input("Photo extraction folder (or press Enter for 'extracted_photos'): ").strip()
        if not extract_dir:
            extract_dir = "extracted_photos"
        
        # If it's just a folder name, try to put it in data/output if exists, otherwise current directory
        if not os.path.dirname(extract_dir):
            data_output_dir = "data/output"
            if os.path.exists(data_output_dir):
                extract_dir = os.path.join(data_output_dir, extract_dir)
...
def main():
    ...
    parser.add_argument('zip_file', nargs='?', default=None, 
                       help='Path to WhatsApp chat export zip file (default: data/input/WhatsApp Chat.zip)')
    ...
    if args.interactive:
        ...
        if interactive_args is None:  # User cancelled
            return 0
    ...
    if args.zip_file is None:
        # Try data/input first, then current directory
        data_input_path = os.path.join("data", "input", "WhatsApp Chat.zip")
        if os.path.exists(data_input_path):
            args.zip_file = data_input_path
        else:
